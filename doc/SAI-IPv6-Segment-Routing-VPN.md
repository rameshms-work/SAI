...
Overlay customer traffic is classified into a VRF and sent to a remote over the underlay core network. A policy is a combination of both color and end node. The end node on a specific policy can be reached via multiple underlay Segment Lists (SL) or tunnels. 
...
### VPN SID
A VPN SID is added to the SID list to the tenant or VRF. The VPN SID value is generated by the end node and propagated through IGP/EGP or via CLI/management tools by a controller.
...
L3VPN only use case is when SRv6 is used to provide L3VPN solution but without a SRv6 path configured.
This allows deployment of SRv6 solution as the network is being upgraded to be SRv6 aware.
...
## SRv6 VPN configuration using Tunnel map example
...
```c
    ...
    // 2.2 Create a tunnel object with source IP used for SRv6 tunnels to NODE21 : NODE21_TNL
